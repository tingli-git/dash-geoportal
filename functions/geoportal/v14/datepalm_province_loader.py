...
from functions.geoportal.v14.cloud_assets import ensure_local_asset, ensure_local_directory, list_directory_names
...
def _simple_geojson_path(province: str) -> Path | None:
    directory = ensure_local_directory(Path(getattr(CFG, "datepalms_province_simple_dir", "")), suffixes=(".geojson",))
    if not directory:
        return None
...
def _province_paths() -> ProvinceMap:
    dir_path = ensure_local_directory(Path(getattr(CFG, "datepalms_province_dir", "")), suffixes=(".gpkg",))
