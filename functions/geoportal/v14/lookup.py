...
from functions.geoportal.v14.cloud_assets import ensure_local_asset, ensure_local_directory
...
class FieldLookup:
    ...
    def __init__(self, province_dir: Path | str, lookup_json: Path | str):
        self.province_dir = ensure_local_directory(Path(province_dir), suffixes=(".gpkg",))
    ...
    def _load_lookup_mapping(self, lookup_json: Path | str) -> Tuple[Dict[int, str], Dict[str, int]]:
        if not lookup_json:
            return {}, {}
        path = ensure_local_asset(Path(lookup_json))
        if not path.exists():
            return {}, {}
