# functions/geoportal/v14/layers.py
from __future__ import annotations
from typing import Iterable, Sequence, Tuple, Optional, Union, List
import ipyleaflet

# 👉 bring in your utils
from functions.geoportal.v14.utils import padded_bounds  # html_table_popup is UI-level; not needed here
...
def _is_base_layer(lyr: ipyleaflet.Layer) -> bool:
    """True iff this is a base tile layer (TileLayer/LocalTileLayer with base=True)."""
    return isinstance(lyr, (ipyleaflet.TileLayer, ipyleaflet.LocalTileLayer)) and bool(getattr(lyr, "base", False))
...
# -----------------------------
# NEW: overlay utilities (generalized for any layer, incl. GeoJSON)
# -----------------------------
def add_overlay_layer(
    m: ipyleaflet.Map,
    overlay: ipyleaflet.Layer,
    *,
    below_markers: bool = True
) -> None:
    """
    Add any overlay (GeoJSON, TileLayer, LayerGroup, etc.) above base layers.
...
def _find_layers_by_name(
    ...
    """
    Return layers whose .name is in names.
    If overlay_only=True, skip base tile layers.
    """
    names = set(names)
    hits: list[ipyleaflet.Layer] = []
    for lyr in m.layers:
        lname = getattr(lyr, "name", "")
        if lname not in names:
            continue
        if overlay_only and _is_base_layer(lyr):
            continue
        hits.append(lyr)
    return hits
...
def set_layer_visibility(
    ...
    """Toggle visibility by add/remove for any layer type."""
