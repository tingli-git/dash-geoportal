...
## 0.65) Run the Geoportal v11 stack
The v11 stack keeps the same infrastructure but points at `functions/geoportal/v11`. Before starting it, run `scripts/normalize_provinces.py` so `/datawaha/esom/DatePalmCounting/Geoportal/Datepalm/app_server/datepalms_provinces.json` exists—the v11 app uses that JSON plus the `field_id`/`province_id` attributes (served through the tiles on port 8766) to look up metadata. Then launch:
```bash
cd /datawaha/esom/Ting/Projects/DatePlamMapping/gitrepo/dash-geoportal/
source .venv/bin/activate
python -m pip install -e .
solara run --production functions/geoportal/v11/app.py
```
The Date Palm Fields product now stays at the national zoom (level 6), uses the green `datepalms_province_*` palette, and shows an info popup for the exact polygon you clicked—no more blue tiles or missing popups. You can still expose the app with the Cloudflare tunnel if you need a shareable link.
...
python scripts/normalize_provinces.py \
  ...
  --mapping datepalms_provinces.json
```
Each rewritten `.gpkg` now contains `field_id`, `province_id`, and `esti_tree_number`, where `esti_tree_number` is computed as `round(area_m2 / 100 * 0.73)` so the value remains an integer. You can reference `datepalms_provinces.json` (which maps each `province_id` to the province name) elsewhere in the dashboard.

### 0.7) Export provinces to GeoJSON (v11)
The v11 product prefers GeoJSON copies of each province file when pre-loaded data is needed. Run the new helper to dump every `.gpkg` from `datepalms_per_province` into `.geojson` files (by default it writes next to the sources, but `--output-dir` lets you pick another folder). Use `--force` to overwrite an existing GeoJSON and `--province <name>` to limit the conversion to a few provinces.
```bash
source .venv/bin/activate
python scripts/convert_province_gpkg_to_geojson.py --province-dir /datawaha/esom/DatePalmCounting/Geoportal/Datepalm/app_server/datepalms_per_province --output-dir /datawaha/esom/DatePalmCounting/Geoportal/Datepalm/app_server/datepalms_per_province_geojson --force
```
